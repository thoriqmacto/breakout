...
return [
    ...
    */
    'commands' => [
        ...
        'automation:broker-summary-daily' => [
            'label' => 'Daily broker summary',
            'description' => 'Bring every broker-summary asset up to the latest valid trading day, fetching whatever range each one is currently missing.',
            'stockbit_bulk' => true,
            'arguments' => [],
            'options' => [
        ...
        'automation:analysis-refresh' => [
            'label' => 'Analysis refresh',
            'description' => 'Recompute features, asset metrics, broker accumulation rollups, watchlist scores and saved strategy runs from the data already imported.',
            'stockbit_bulk' => false,
            'arguments' => [],
            'options' => [
                'date' => ['type' => 'date', 'label' => 'Recompute for this date only'],
                'from' => ['type' => 'date', 'label' => 'Earliest feature date to rebuild'],
                'max-days' => ['type' => 'integer', 'min' => 1, 'max' => 365, 'label' => 'Most days of features one run rebuilds'],
                'symbol' => ['type' => 'symbol_list', 'label' => 'Restrict to specific tickers'],
                'skip-features' => ['type' => 'boolean', 'label' => 'Do not rebuild features_daily'],
                'skip-metrics' => ['type' => 'boolean', 'label' => 'Do not recompute asset metrics'],
    ...
    */
    'defaults' => [
        ...
        [
            ...
            'slug' => 'daily-analysis-refresh',
            'description' => 'Every day at 18:00 WIB, after both scrapes have landed, recompute everything derived from them: features_daily, asset metrics, broker accumulation rollups, watchlist scores and the saved strategy runs. Priority 30 puts it last in the same dispatcher pass, so it always sees the day\'s imported data rather than yesterday\'s. Condition "none" on purpose: it reads only what is already stored, costs no API quota, and is a no-op when nothing has changed -- so on a day the calendar cannot confirm, it still picks up data that arrived late rather than skipping alongside the scrapes.',
            'command' => 'automation:analysis-refresh',
            'parameters' => ['arguments' => [], 'options' => []],
            'cron_expression' => '0 18 * * *',
            'condition' => 'none',
            'priority' => 30,
